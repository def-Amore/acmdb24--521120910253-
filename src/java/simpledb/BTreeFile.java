...
public class BTreeFile implements DbFile {
     ...
     * nodes along the path to the leaf node with READ_WRITE permission, and locks the
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        if (pid.pgcateg() == BTreePageId.INTERNAL) {
            BTreeInternalPage internalPage = (BTreeInternalPage) getPage(tid, dirtypages, pid, Permissions.READ_WRITE);
            Iterator<BTreeEntry> iter = internalPage.iterator();
            BTreePageId next = null;
            ...
            if (f == null)
                next = internalPage.iterator().next().getLeftChild();
            else if (next == null)
                next = entry.getRightChild();
            return findLeafPage(tid, dirtypages, next, perm, f);
        } else {
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        Tuple mid = page.getTuple(page.getNumTuples() / 2);
        Iterator<Tuple> start = page.iterator();
        Iterator<Tuple> end = page.reverseIterator();
        Tuple temp = null;
        ...
        if (page.getRightSiblingId() == null) {
            ...
            BTreeLeafPage rightPage = (BTreeLeafPage) getPage(tid, dirtypages, newPage.getRightSiblingId(), Permissions.READ_WRITE);
        ...
        BTreeEntry e = new BTreeEntry(mid.getField(keyField), page.getId(), newPage.getId());
        ...
        return field.compare(Op.LESS_THAN, mid.getField(keyField)) ? page : newPage;
    }
    ...
    protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        Iterator<BTreeEntry> start = page.iterator();
        Iterator<BTreeEntry> end = page.reverseIterator();
        BTreeEntry mid = null;
    ...
    private void updateParentPointer(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, BTreePageId child)
        ...
        BTreePage p = (BTreePage) getPage(tid, dirtypages, child, Permissions.READ_WRITE);
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int num = (sibling.getNumTuples() - page.getNumTuples()) / 2;
        BTreeLeafPage leaf;
        Iterator<Tuple> iter;
        if (isRightSibling) {
            leaf = sibling;
        ...
        for (int i = 1; i <= num; i++) {
            if (!iter.hasNext()) {
                throw new DbException("BTreeFile_stealFromLeafPage_hasBext=false");
            }
            Tuple tmp = iter.next();
        ...
        if (leaf.getNumTuples() > 0) {
            entry.setKey(leaf.iterator().next().getField(keyField));
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        for (int i = 1; i <= num; i++) {
            if (!iter.hasNext()) throw new DbException("BTreeFile_stealFromLeftInternalPage_iterhasNext=false");
            ...
            parent.updateEntry(entry);
            parentEntry.setLeftChild(pageId);
            if (!page.iterator().hasNext()) {
                throw new DbException("BTreeFile_stealFromLeftInternalPage_pagehasNext=false");
            }
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        assert curEntries < sibEntries;
        int newCurEntries = (curEntries + sibEntries) >> 1;
        ...
        for (int i = 0; i < newCurEntries - 1 - curEntries; i++) {
            BTreeEntry tmp = iter.next();
            rightSibling.deleteKeyAndLeftChild(tmp);
            page.insertEntry(tmp);
        }
        updateParentPointers(tid, dirtypages, page);
        BTreeEntry root = iter.next();
        rightSibling.deleteKeyAndLeftChild(root);
        parentEntry.setKey(root.getKey());
    ...
    protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        for (Iterator<Tuple> iter = rightPage.iterator(); iter.hasNext(); ) {
            Tuple tmp = iter.next();
        ...
        if (rightPage.getRightSiblingId() != null) {
            BTreeLeafPage rightSiblingPage = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_ONLY);
    ...
    protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        for (Iterator<BTreeEntry> iter = rightPage.iterator(); iter.hasNext(); ) {
            BTreeEntry tmp = iter.next();
            rightPage.deleteKeyAndLeftChild(tmp);
            leftPage.insertEntry(tmp);
        }
...
class BTreeSearchIterator extends AbstractDbFileIterator {
    ...
    protected Tuple readNext() throws TransactionAbortedException, DbException,
        ...
        while (it != null) {
            ...
            BTreePageId next = curp.getRightSiblingId();
            if(next == null) {
                return null;
            }
            else {
                curp = (BTreeLeafPage) Database.getBufferPool().getPage(tid,
                        next, Permissions.READ_WRITE);
                it = curp.iterator();
            }
        }
