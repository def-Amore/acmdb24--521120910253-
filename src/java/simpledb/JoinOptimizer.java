...
public class JoinOptimizer {
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= joins.size(); i++) {
            Set<Set<LogicalJoinNode>> tmp = this.enumerateSubsets(joins, i);
            for (Set<LogicalJoinNode> s : tmp) {
                Double max = Double.MAX_VALUE;
                CostCard cost = new CostCard();
                for (LogicalJoinNode j : s) {
                    CostCard _cost = this.computeCostAndCardOfSubplan(stats, filterSelectivities, j, s, max, cache);
                    if (_cost != null) {
                        max = _cost.cost;
                        cost = _cost;
                    }
                }
                cache.addPlan(s, cost.cost, cost.card, cost.plan);
            }
        }
        return cache.bestOrders.get(enumerateSubsets(joins, joins.size()).iterator().next());
    }
